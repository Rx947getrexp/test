...
type UploadPaymentQRCodeReq struct {
...
type UploadPaymentQRCodeRes struct {
	Url string `json:"url" dc:"地址"`
}

func UploadPaymentQRCode(ctx *gin.Context) {
	var (
		err     error
		req     = new(UploadPaymentQRCodeReq)
		fileUrl string
	)
	...
	}, fileMap, constant.UploadFilePathPayment)
	if err != nil {
	...
	response.RespOk(ctx, i18n.RetMsgSuccess, UploadPaymentQRCodeRes{Url: fileUrl})
}

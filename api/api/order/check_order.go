...
func CheckOrder(ctx *gin.Context) {
	...
	_, err = dao.TPayOrder.Ctx(ctx).Where("user_id = ? and order_no = ? and payment_channel_name = ?", req.UserId, req.OrderNo, req.PaymentChannelName).One(&order)
	if err != nil {
	...
	if order.Status == "fail" {
		...
		giveFreeToUser(ctx, req.UserId, req.PaymentChannelName)
		return
	}
	res := CheckOrderRes{

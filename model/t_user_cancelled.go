...
type TUserCancelled struct {
	...
	LastLoginIp      string    `xorm:"comment('lastLoginIp') VARCHAR(32)"`
	LastLoginCountry string    `xorm:"comment('lastLoginCountry') VARCHAR(32)"`
	PreferredCountry string    `xorm:"comment('preferredCountry') VARCHAR(64)"`
	Version          int       `xorm:"comment('Version') INT"`
}

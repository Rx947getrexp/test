...
class Speed:
    ...
    def count_table(self, table_name, days):
        sql = """select count(*) as cnt FROM %s WHERE created_at < DATE_SUB(NOW(), INTERVAL %d DAY);""" % (table_name, days)
        rows = mysql_query_db(self.conn, sql)
        if len(rows) != 1:
            logging.error("sql: %s, rows: %d != 1" % (sql, len(rows)))
            sys.exit(1)
    ...
    def clean_data(self, table_name, days):
        count = self.count_table(table_name, days)
        if count == 0:
            return
        logging.info("clean_data_count: table(%s), days(%d), count(%d)" % (table_name, days, count))
        for i in range(int(count/100)+1):
            logging.info("loop(%s): %d" % (table_name, i))
            sql = """DELETE FROM %s WHERE created_at < DATE_SUB(NOW(), INTERVAL %d DAY) limit 100;""" % (table_name, days)
